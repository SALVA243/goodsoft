"use client";
import Produit_data from "@/data/ProduitsData";
import Image from "next/image";
import Link from "next/link";
import { useState } from "react";

const ProduitsDivers = () => {
  const [currentPage, setCurrentPage] = useState(1);

  const blogs = Produit_data.filter((items) => items.type === "divers");
  ...
  return (
    <div className="our-Events-page rel z-1">
      <div className="container">
        <div className="row justify-content-center">
          {currentItems.map((item) => (
            <div key={item.id} className="col-xl-4 col-md-6 py-40">
              <div className="image">
                <Image src={item.ThumbDetail} alt="Event" />
              </div>
